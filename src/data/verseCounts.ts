// Tabela estática com contagem exata de versículos por capítulo
// Cobre todos os 73 livros da Bíblia Católica
// Usar getVerseCount(livro, capítulo) para obter o total de versículos

export const VERSE_COUNTS: Record<string, number[]> = {
  // ── Pentateuco ──────────────────────────────────────────────────────────────
  'Gênesis':       [31,25,24,26,32,22,24,22,29,32,32,20,18,24,21,16,27,33,38,18,34,24,20,67,34,35,46,22,35,43,55,32,20,31,29,43,36,30,23,23,57,38,34,34,28,34,31,22,33,26],
  'Êxodo':         [22,25,22,31,23,30,25,32,35,29,10,51,22,31,27,36,16,27,25,26,36,31,33,18,40,37,21,43,46,38,18,35,23,35,35,38,29,31,43,38],
  'Levítico':      [17,16,17,35,19,30,38,36,24,20,47,8,59,57,33,34,16,30,24,16,30,24,26,27,32,25],
  'Números':       [54,34,51,49,31,27,89,26,23,36,35,16,33,45,41,50,13,32,22,29,35,41,30,25,18,65,23,31,40,16,54,42,56,29,34,13],
  'Deuteronômio':  [46,37,29,49,33,25,26,20,29,22,32,32,18,29,23,22,20,22,21,20,23,30,25,22,19,19,26,68,29,20,30,52,29,12],
  // ── Históricos ──────────────────────────────────────────────────────────────
  'Josué':         [18,24,17,24,15,27,26,35,27,43,23,24,33,15,63,10,18,28,51,9,45,34,16,33],
  'Juízes':        [36,23,31,24,31,40,25,35,57,18,40,15,25,20,20,31,13,31,30,48,25],
  'Rute':          [22,23,18,22],
  '1 Samuel':      [28,36,21,22,12,21,17,22,27,27,15,25,23,52,35,23,58,30,24,42,15,23,29,22,44,25,12,25,11,31,13],
  '2 Samuel':      [27,32,39,12,25,23,29,18,13,19,27,31,39,33,37,23,29,33,43,26,22,51,39,25],
  '1 Reis':        [53,46,28,34,18,38,51,66,28,29,43,33,34,31,34,34,24,46,21,43,29,53],
  '2 Reis':        [18,25,27,44,27,33,20,29,37,36,21,21,25,29,38,20,41,37,37,21,26,20,37,20,30],
  '1 Crônicas':    [54,55,24,43,26,81,40,40,44,14,47,40,14,17,29,43,27,17,19,8,30,19,32,31,31,32,34,21,30],
  '2 Crônicas':    [17,18,17,22,14,42,22,18,31,19,23,16,22,15,19,14,19,34,11,37,20,12,21,27,28,23,9,27,36,27,21,33,25,33,27,23],
  'Esdras':        [11,70,13,24,17,22,28,36,15,44],
  'Neemias':       [11,20,32,23,19,19,73,18,38,39,36,47,31],
  'Tobias':        [22,14,17,21,22,18,16,21,22,27,36,23,12,22],
  'Judite':        [16,28,10,15,24,21,32,36,14,23,23,20,20,19,13,25],
  'Ester':         [22,17,19,11,16,16,18,23,20,9],
  '1 Macabeus':    [64,70,60,61,68,63,50,32,73,89,74,53,53,49,41,24],
  '2 Macabeus':    [36,32,40,50,27,31,42,36,29,38,38,45,26,46,39],
  // ── Sapienciais ─────────────────────────────────────────────────────────────
  'Jó':            [22,17,21,11,13,21,22,17,18,24,24,18,31,31,26,31,31,34,22,26,25,23,17,27,22,21,21,27,23,15,18,14,30,40,10,38,24,22,17,17,11,14],
  'Salmos':        [6,12,8,8,12,10,17,9,20,18,7,8,6,7,5,11,15,50,14,9,13,31,6,10,22,12,14,9,11,13,25,11,22,23,28,13,40,23,14,18,14,12,5,27,18,12,10,15,21,23,21,11,7,9,24,14,12,12,18,14,9,13,12,11,14,20,8,36,37,6,24,20,28,23,11,13,21,72,13,20,17,8,19,13,14,17,7,19,53,17,16,16,5,23,11,13,12,9,9,5,8,28,22,35,45,48,43,13,31,7,10,10,9,8,18,19,2,29,176,7,8,9,4,8,5,6,5,6,8,8,3,18,3,3,21,26,9,8,24,14,10,8,12,15,21,10,20,14,9,6],
  'Provérbios':    [33,22,35,27,23,35,27,36,18,32,31,28,25,35,33,33,28,24,29,30,31,29,35,34,28,28,27,28,27,33,31],
  'Eclesiastes':   [18,26,22,16,20,12,29,17,18,20,10,14],
  'Cântico dos Cânticos': [17,17,11,16,16,13,13,14],
  'Sabedoria':     [16,24,19,20,23,25,30,21,18,21,26,27,19,31,19,29,21,25,22],
  'Eclesiástico':  [30,18,31,31,15,37,36,19,18,31,34,18,26,27,20,30,32,33,30,52,29,12,50,33,33,28,26,29,30,26,20,29,22,32,32,30,25,14,14,22,18,24,20,24,25,23,22,22,21,22,23,22,22,21,24,22,23,22,27,22,25,25,27,24,21,21,25,25,22,21,22,22,22,22,14,22,22,22,29,22],
  // ── Proféticos ──────────────────────────────────────────────────────────────
  'Isaías':        [31,22,26,6,30,13,25,22,21,34,16,6,22,32,9,14,14,7,25,6,17,25,18,23,12,21,13,29,24,33,9,20,24,17,10,22,38,22,8,31,29,25,28,28,25,13,15,22,26,11,23,15,12,17,13,12,21,14,21,22,11,12,19,12,25,24],
  'Jeremias':      [19,37,25,31,31,30,34,22,26,25,23,17,27,22,21,21,27,23,15,18,14,30,40,10,38,24,22,17,17,11,14,20,28,36,24,30,32,25,26,39,20,13,17,12,19,21,18,31,20,20,14,27,13,23,25,26,33,17,19,22,20,27,12,21,14,5],
  'Lamentações':   [22,22,66,22,22],
  'Baruc':         [21,62,30,24,49,30],
  'Ezequiel':      [28,10,27,21,17,21,22,22,27,27,21,11,16,16,13,17,13,31,22,6,21,15,22,31,17,21,32,19,24,46,23,22,29,29,22,16,28,22,29,32,32,20,22,30,20,20,25,28,22,22,28,24],
  'Daniel':        [21,49,30,37,31,28,28,27,27,21,45,13,65,19],
  'Oseias':        [11,23,5,19,15,11,16,14,17,15,12,14,16,9],
  'Joel':          [20,27,33,21],
  'Amós':          [15,16,15,13,27,14,17,14,15],
  'Abdias':        [21],
  'Jonas':         [17,10,10,11],
  'Miqueias':      [16,13,12,13,15,16,20],
  'Naum':          [15,13,19],
  'Habacuc':       [17,20,19],
  'Sofonias':      [18,15,20],
  'Ageu':          [15,23],
  'Zacarias':      [21,13,10,14,11,15,14,23,17,12,17,14,9,21],
  'Malaquias':     [14,17,18],
  // ── Novo Testamento ─────────────────────────────────────────────────────────
  'Mateus':        [25,23,17,25,48,34,29,34,38,42,30,50,58,36,39,28,27,35,30,34,46,46,39,51,46,75,66,20],
  'Marcos':        [45,28,35,41,43,56,37,38,50,52,33,44,37,72,47,20],
  'Lucas':         [80,52,38,44,39,49,50,56,62,42,54,59,35,35,32,31,37,43,48,47,38,71,56,53],
  'João':          [51,25,36,54,47,71,53,59,41,42,57,50,38,31,27,33,26,40,42,31,25],
  'Atos':          [26,47,26,37,42,15,60,40,43,48,30,25,52,28,41,40,34,28,41,38,40,30,35,27,27,32,44,31],
  'Romanos':       [32,29,31,25,21,23,25,39,33,21,36,21,14,23,33,27],
  '1 Coríntios':   [31,16,23,21,13,20,40,34,29,27,36,27,21,35,33,31],
  '2 Coríntios':   [24,17,18,18,21,18,16,24,15,18,33,21,14],
  'Gálatas':       [24,21,29,31,26,18],
  'Efésios':       [23,22,21,28,22,24],
  'Filipenses':    [30,30,21,23],
  'Colossenses':   [29,23,25,18],
  '1 Tessalonicenses': [10,20,13,18,28],
  '2 Tessalonicenses': [12,17,18],
  '1 Timóteo':     [20,15,16,16,25,21],
  '2 Timóteo':     [18,26,17,22],
  'Tito':          [16,15,15],
  'Filémon':       [25],
  'Hebreus':       [14,18,19,16,14,20,28,13,28,39,40,29,25],
  'Tiago':         [27,26,18,17,20],
  '1 Pedro':       [25,25,22,19,14],
  '2 Pedro':       [21,22,18],
  '1 João':        [10,29,24,21,21],
  '2 João':        [13],
  '3 João':        [15],
  'Judas':         [25],
  'Apocalipse':    [20,29,22,11,14,17,17,13,21,11,19,17,18,20,8,21,18,24,21,15,27,21],
};

/**
 * Retorna o número de versículos em um determinado capítulo de um livro.
 * @param bookName Nome do livro em português (ex: "João")
 * @param chapter  Número do capítulo (1-based)
 * @returns        Total de versículos; 30 como fallback seguro se não encontrado
 */
export function getVerseCount(bookName: string, chapter: number): number {
  const counts = VERSE_COUNTS[bookName];
  if (!counts || chapter < 1 || chapter > counts.length) return 30;
  return counts[chapter - 1];
}

/**
 * Retorna o total de capítulos de um livro.
 */
export function getChapterCount(bookName: string): number {
  const counts = VERSE_COUNTS[bookName];
  return counts ? counts.length : 1;
}
