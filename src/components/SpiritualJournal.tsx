...
export default function SpiritualJournal() {
  ...
  // Colapso de grupos de tipo: false = expandido (padrão), true = recolhido
  const [collapsedTypes, setCollapsedTypes] = useState<Record<string, boolean>>({});

  // Colapso de cards individuais: true = recolhido (padrão ao carregar)
  // Ao carregar registros, todos os cards ficam minimizados (só título visível)
  ...
  // ── Colapso de grupos (tipos) ─────────────────────────────────────────────
  const toggleTypeCollapse = (t: string) => {
    setCollapsedTypes(prev => ({ ...prev, [t]: !prev[t] }));
  };

  // ── Colapso de cards individuais ──────────────────────────────────────────
  const toggleCardCollapse = (id: number) => {
    setCollapsedCards(prev => ({ ...prev, [id]: !prev[id] }));
  };

  // ── Expandir / Recolher TUDO (tipos + cards) ──────────────────────────────
  const toggleAllCollapse = (collapse: boolean) => {
    // Colapsa/expande todos os grupos de tipo
    const nextTypes: Record<string, boolean> = {};
    Object.keys(typeLabels).forEach(t => { nextTypes[t] = collapse; });
    setCollapsedTypes(nextTypes);

    // Colapsa/expande todos os cards individuais
    const nextCards: Record<number, boolean> = {};
    entries.forEach(e => { nextCards[e.id] = collapse; });
  ...
  // Agrupar por tipo e ordenar: data DESC, desempate título A-Z
    ...
    <div className="space-y-8">
      ...
      <AnimatePresence>
        {isAdding && (
          ...
          >
            ...
            {/* CORREÇÃO: onSubmit no form garante funcionamento pelo botão E pelo Enter */}
            <form onSubmit={handleSubmit} className="space-y-5">

              {/* Tipo */}
              <div>
                <label className="text-xs font-bold uppercase tracking-widest text-[#5A5A40] mb-2 block">Tipo</label>
              ...
              {/* Título */}
              <div>
                <label className="text-xs font-bold uppercase tracking-widest text-[#5A5A40] mb-1 block">
                  Título
                </label>
                <input
                  type="text"
              ...
              <div className="flex justify-end gap-3">
                ...
                {/* CORREÇÃO: type="submit" garante disparo do onSubmit */}
                <button
                  type="submit"
                  disabled={!newContent.trim() || isSaving}
                  className="px-8 py-3 bg-[#5A5A40] text-white rounded-2xl font-bold shadow-lg shadow-[#5A5A40]/20 hover:scale-105 transition-all flex items-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed"
                >
                  {isSaving
                    ? <><Loader2 className="w-4 h-4 animate-spin" /> Salvando...</>
                    : <><Save className="w-4 h-4" /> {editingEntry ? 'Atualizar' : 'Salvar'}</>
                  }
                </button>
              </div>
            </form>
      ...
      <section className="space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {/* Expandir / Recolher Tudo — afeta grupos E cards */}
          ...
          <button
            ...
            className={`px-3 py-2 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all ${
              selectMode
                ...
                : 'bg-[#F5F2ED] text-[#5A5A40] hover:bg-[#5A5A40]/10'
            }`}
          >
          ...
          <div className="space-y-8">
            {Object.keys(groupedEntries).map((t) => (
              <div key={t} className="space-y-3">
                ...
                {/* ── Cabeçalho do grupo (tipo) — clicável para expandir/recolher o grupo ── */}
                <button
                  type="button"
                    ...
                    >
                      <div className="grid gap-3 py-1">
                        {groupedEntries[t].map((entry) => {
                          const isCardCollapsed = collapsedCards[entry.id] !== false;
                          // Por padrão (undefined ou true) = recolhido; false = expandido
                          return (
                            <motion.div
                              key={entry.id}
                              layout
                              className="bg-white rounded-[1.75rem] border border-[#1A1A1A]/5 shadow-sm hover:shadow-md transition-all group overflow-hidden"
                            >
                              {/* ── Cabeçalho do card — clicável para expandir/recolher o card ── */}
                              <div
                                className="flex items-center justify-between px-6 py-4 cursor-pointer select-none"
                                onClick={() => !selectMode && toggleCardCollapse(entry.id)}
                              >
                                <div className="flex items-center gap-3 flex-1 min-w-0">
                                  ...
                                  {selectMode && (
                                    <button
                                      type="button"
                                      onClick={(ev) => { ev.stopPropagation(); toggleSelect(entry.id); }}
                                      className="shrink-0"
                                    >
                                  ...
                                  <div className="min-w-0 flex-1">
                                    <p className="font-bold text-sm text-[#1A1A1A] truncate leading-snug">
                                      {entry.title?.trim() || <span className="text-[#1A1A1A]/30 italic font-normal">(sem título)</span>}
                                    </p>
                                    <div className="flex items-center gap-1.5 text-[#1A1A1A]/35 text-xs mt-0.5">
                                ...
                                {/* Ações + indicador de colapso */}
                                <div className="flex items-center gap-1 shrink-0 ml-2">
                                  {!selectMode && (
                                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                      <button
                                        type="button"
                                        onClick={(ev) => { ev.stopPropagation(); exportSinglePDF(entry); }}
                                        title="Exportar PDF"
                                        className="p-1.5 hover:bg-[#F5F2ED] rounded-full text-[#5A5A40]/50 transition-colors"
                                      >
                                      ...
                                      <button
                                        ...
                                        className="p-1.5 hover:bg-[#F5F2ED] rounded-full text-[#5A5A40] transition-colors"
                                      >
                                        <PenTool className="w-3.5 h-3.5" />
                                      </button>
                                      <button
                                        type="button"
                                        onClick={(ev) => { ev.stopPropagation(); setDeleteId(entry.id); }}
                                  ...
                                  {/* Seta de colapso do card */}
                                  <div className="p-1.5 text-[#1A1A1A]/20">
                                    {isCardCollapsed
                              ...
                              {/* ── Conteúdo do card (expande/recolhe) ── */}
                              <AnimatePresence>
                                {!isCardCollapsed && (
                                  <motion.div
                                    initial={{ height: 0, opacity: 0 }}
                                  ...
                                  >
                                    <div className="px-6 pb-6 pt-0">
                                      <div className="h-px bg-[#1A1A1A]/5 mb-4" />
