...
function lsSet(base: string, value: unknown) {
  try { localStorage.setItem(getUserKey(base), JSON.stringify(value)); } catch {}
}
...
Seja profundo, fiel à doutrina católica e didático.`,
  ...
  async function startLectio() {
    ...
    try {
      ...
      if (!data?.leitura) { alert('Não foi possível gerar o guia. Tente novamente em alguns instantes.'); return; }
      setLectioData(data);
  ...
  function handleSave() {
    ...
    try {
      const existing = lsGet<HistoryItem[]>('lectio_history', []);
  ...
  function revisitar(item: HistoryItem) {
        ...
        <div className="bg-white p-8 sm:p-12 rounded-[3rem] border border-[#1A1A1A]/5 shadow-sm">
          <div className="max-w-2xl mx-auto space-y-8">
            <div>
              ...
              <p className="text-[#1A1A1A]/60 text-sm">Exegese Católica Completa — versículo por versículo, com contexto histórico, palavras-chave, interpretação dos santos e aplicação espiritual.</p>
            </div>
            ...
            <button onClick={fetchExegesis} disabled={isLoadingExegesis}
              ...
              {isLoadingExegesis ? <><Loader2 className="w-5 h-5 animate-spin" /> Analisando...</> : '🔍 Gerar Exegese Completa'}
            </button>
            {isLoadingExegesis && (
            ...
            {exegesis && !isLoadingExegesis && (
              <div className="bg-[#F5F2ED] p-8 rounded-[2rem] border border-[#5A5A40]/10">
                <div className="flex items-center gap-3 mb-6">
                  <div className="px-4 py-2 bg-[#5A5A40] text-white rounded-full text-xs font-bold uppercase tracking-widest">
                    {selectedBook.name} {selectedChapter}:{startVerse}–{endVerse}
                  </div>
                  <span className="text-xs text-[#1A1A1A]/40">Exegese Católica Completa</span>
                </div>
                <div className="text-base leading-relaxed text-[#1A1A1A]/80 font-serif whitespace-pre-wrap prose prose-sm max-w-none">
                  {exegesis}
                </div>
              </div>
            )}
